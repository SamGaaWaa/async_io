...
namespace async {
    namespace net {
        namespace tcp {
            ...
            template<class Executor, bool Block = false>
            class socket {
                public:
                ...
                auto async_accept(CallBack&& callback) {
                    ...
                    _executor.async_accept(_fd, std::forward<CallBack>(callback));
                }

                template<class CallBack>
                auto async_read(auto* buff, size_t max_size, CallBack&& callback) {
                    static_assert(!Block, "Only support nonblocking socket.\n");
                    _executor.async_read_some(_fd, buff, max_size, std::forward<CallBack>(callback));
                }
